...
function sopac_user_info_table(&$account, &$locum) {
  ...
  if (variable_get('sopac_email_enable', 1)) {
    ...
    if ($userinfo['pnum']) {
      ...
      $link_text = ($userinfo['email'] ? $userinfo['email'] : 'Add a notification email address');
      $email_link = l($link_text, 'user/' . $account->uid . '/edit/notifications');
...
function sopac_fine_payment_form() {
  ...
  $form['sopac_payment_cc_info']['ccnum'] = array(
    ...
    '#description' => t('Your Visa, MasterCard, or Discover Number (Amex not accepted)'),
    '#size' => 24,
    '#maxlength' => 20,
    '#required' => TRUE,
  );
